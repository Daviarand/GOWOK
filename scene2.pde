...
class Scene2 {
  ...
  // Variabel Subtitle
  String adilInitialSubtitle = "Adil: Untuk apa aku terus hidup...";
  String[] subtitleListUstadzWalking, subtitleList;
  int[] subtitleDurationsUstadzWalking, subtitleDurations;
  int currentWalkingSubtitleIndex, currentSubtitleIndex;
  
  // Variabel Pembantu & State
  int lastSubtitleChangeTime; // KOREKSI: Diubah dari long menjadi int
  int charsToShow;
  String currentFullText;
  float ustadzX, ustadzY;
  boolean ustadzBerjalan, conversationStarted;
  int frameCounter;
  final int frameInterval = 10; 
  final float scale = 0.4f;
  
  long delayStartTime = -1;
  ...
  void reset() {
    ...
    delayStartTime = -1;
    ustadzX = p.width; 
    ustadzY = 630 - (ustadzJalan1.height * scale);
  ...
  void run() {
    ...
    if (!conversationStarted) {
      ...
      else if (p.millis() - delayStartTime > 2000) {
        drawWalkingSubtitle();
      }
    } else {
      drawConversationSubtitle();
    }
    
    if (currentSubtitleIndex == subtitleList.length - 1) {
      if (p.millis() - lastSubtitleChangeTime > 2000) {
        // KOREKSI: Menggunakan nama sketch yang benar
  ...
  int[] calculateDurations(String[] list, int baseTime, int timePerChar) {
    int[] durations = new int[list.length];
    for(int i=0; i<list.length; i++) durations[i] = baseTime + (list[i].length() * timePerChar);
    return durations;
  }
  ...
  void initializeSubtitles() {
    ...
    subtitleDurationsUstadzWalking = calculateDurations(subtitleListUstadzWalking, 1500, 75);
  }
  
  void drawInitialSubtitle() {
    drawSubtitleBox(adilInitialSubtitle);
    if (p.millis() - lastSubtitleChangeTime > 4000) {
      delayStartTime = p.millis();
      lastSubtitleChangeTime = p.millis(); // Reset timer untuk jeda
    }
  }

  void drawWalkingSubtitle() {
    if (subtitleListUstadzWalking[currentWalkingSubtitleIndex].isEmpty()) return;
    if (p.millis() - lastSubtitleChangeTime > subtitleDurationsUstadzWalking[currentWalkingSubtitleIndex]) {
      if (currentWalkingSubtitleIndex < subtitleListUstadzWalking.length - 1) {
        currentWalkingSubtitleIndex++;
        currentFullText = "";
      }
    }
    String fullText = subtitleListUstadzWalking[currentWalkingSubtitleIndex];
    drawSubtitleBox(fullText);
  }

  void drawConversationSubtitle() {
  ...
  void drawSubtitleBox(String fullText) {
  ...
  void drawWaterWaves(){p.noFill();for(int y_base=waterLevel;y_base<waterBottom;y_base+=7){float p_val=p.map(y_base,waterLevel,waterBottom,0,1),amp=p.lerp(1.5f,4,p_val),speed=p.lerp(0.02f,0.04f,p_val);p.strokeWeight(p.lerp(1.5f,2.5f,p_val));p.stroke(255,p.map(p_val,0,1,90,40));float startX=p.map(y_base,waterLevel,waterBottom,leftBankTopX,leftBankBottomX),endX;if(y_base<waterUpperMidY)endX=p.map(y_base,waterLevel,waterUpperMidY,rightBankTopX,rightBankUpperMidX);else if(y_base<waterMidY)endX=p.map(y_base,waterUpperMidY,waterMidY,rightBankUpperMidX,rightBankMidX);else endX=p.map(y_base,waterMidY,waterBottom,rightBankMidX,rightBankBottomX);p.beginShape();for(float x=startX;x<=endX;x+=10)p.vertex(x,y_base+p.sin(x*waveFrequency+p.frameCount*speed)*amp);p.endShape();}}
